...
- (id) initWithFrame:(NSRect)frame
{
    ...
    self = [super initWithFrame:frame pixelFormat:gSharedPixelFormat];
    ...
    BOOL shouldUseRetina = [NSUserDefaults.standardUserDefaults boolForKey:@"UseRetinaResolution"];
    ...
    if ([NSUserDefaults.standardUserDefaults objectForKey:@"Framerate"] != nil)
        _framerate = (NSTimeInterval)[NSUserDefaults.standardUserDefaults doubleForKey:@"Framerate"];
    else
        _framerate = 60.0;
    ...
    _highQualityBackgroundWindows = [NSUserDefaults.standardUserDefaults boolForKey:@"BackgroundWindowsRenderAtFullQuality"];
    ...
    if (gInvisibleCursor == nil)
    {
        ...
        if (invisibleImage == nil)
            ExitGracefully([NSString stringWithFormat:@"The image %@.%@ could not be loaded.", filename, extension]);
